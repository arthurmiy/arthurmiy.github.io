...
console.log(
  "Potree " +
    Potree.version.major +
    "." +
    Potree.version.minor +
    Potree.version.suffix
);
...
Potree.toMaterialID = function (materialName) {
  if (materialName === "RGB") {
  ...
  } else if (materialName === "Cor") {
    return Potree.PointColorType.COLOR;
  } else if (materialName === "Elevação") {
    return Potree.PointColorType.HEIGHT;
  } else if (materialName === "Distancia ao cabo") {
  ...
  } else if (materialName === "Classificação") {
  ...
  } else if (materialName === "Nível de detalhe") {
    ...
    return Potree.PointColorType.POINT_INDEX;
  } else if (materialName === "RGB e Elevação") {
    return Potree.PointColorType.RGB_HEIGHT;
  } else if (materialName === "Composição") {
  ...
  while (priorityQueue.size() > 0) {
    ...
    if (false && pointcloud.material.clipBoxes.length > 0) {
      ...
      if (pointcloud.material.clipTask === Potree.ClipTask.SHOW_INSIDE) {
        if (
          pointcloud.material.clipMethod === Potree.ClipMethod.INSIDE_ANY &&
          insideAny
        ) {
          //node.debug = true
        } else if (
          pointcloud.material.clipMethod === Potree.ClipMethod.INSIDE_ALL &&
          insideAll
        ) {
          //node.debug = true;
        } else {
  ...
  update() {
    ...
    if (texture instanceof THREE.DataTexture) {
      ...
      gl.texParameteri(
        ...
        Potree.paramThreeToGL(gl, texture.magFilter)
      );
      gl.texParameteri(
        this.target,
      ...
      gl.texParameteri(
        this.target,
        gl.TEXTURE_WRAP_S,
        Potree.paramThreeToGL(gl, texture.wrapS)
      );
      gl.texParameteri(
        this.target,
        gl.TEXTURE_WRAP_T,
        Potree.paramThreeToGL(gl, texture.wrapT)
      );
    ...
    for (let node of nodes) {
...

      {
        ...
        if (material.clipPolygons && material.clipPolygons.length > 0) {
          ...
          let flattenedMatrices = [].concat(
            ...worldViewProjMatrices.map((m) => m.elements)
          );

          let flattenedVertices = new Array(
      ...
      if (shadowMaps.length > 0) {
        ...
        shader.setUniform3f(
          "uShadowColor",
          material.uniforms.uShadowColor.value
        );

        let bindingStart = 5;
        let bindingPoints = new Array(shadowMaps.length)
          .fill(bindingStart)
          .map((a, i) => a + i);
        ...
        for (let i = 0; i < shadowMaps.length; i++) {
          ...
          let glTexture = this.threeRenderer.properties.get(
        ...
        {
          let flattenedMatrices = [].concat(
            ...shadowMaps.map((sm) => sm.camera.projectionMatrix.elements)
          );
  ...
  renderOctree(octree, nodes, camera, target, params = {}) {
...

    {
      ...
      if (material.clipBoxes && material.clipBoxes.length > 0) {
        let flattenedMatrices = [].concat(
          ...material.clipBoxes.map((c) => c.inverse.elements)
        );
      ...
      let classificationTexture = this.textures.get(
      ...
      if (material.snapEnabled === true) {
        {
          ...
          let lSnapshotBindingPoints = new Array(5)
            .fill(bindingStart)
            .map((a, i) => a + i);
          ...
          for (let i = 0; i < 5; i++) {
            ...
            let snapTexture = this.threeRenderer.properties.get(texture)
              .__webglTexture;
            let snapTextureDepth = this.threeRenderer.properties.get(
              textureDepth
            ).__webglTexture;
      ...
      for (let demNode of targetNodes) {
        ...
        for (let i = 0; i < this.tileSize; i++) {
          for (let j = 0; j < this.tileSize; j++) {
            ...
            let ix =
              (this.tileSize * (x - projectedBox.min.x)) / projectedBoxSize.x;
            let iy =
              (this.tileSize * (y - projectedBox.min.y)) / projectedBoxSize.y;
...
float getIntensity(){
	float w = (intensity - intensityGamma) / (intensityGamma - intensityGamma);
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && (xhr.status === 200 || xhr.status === 0)) {
        ...
        if (fMno.pointAttributes === "LAS") {
          ...
          pco.loader = new Potree.BinaryLoader(
            fMno.version,
            boundingBox,
            fMno.scale
          );
...

        {
          ...
          let root = new Potree.PointCloudOctreeGeometryNode(
            name,
            pco,
            boundingBox
          );
        ...
        if (version.upTo("1.4")) {
          for (let i = 1; i < fMno.hierarchy.length; i++) {
            ...
            let boundingBox = Potree.POCLoader.createChildAABB(
              parentNode.boundingBox,
              index
            );

            let node = new Potree.PointCloudOctreeGeometryNode(
              name,
              pco,
              boundingBox
            );
    ...
    xhr.onreadystatechange = () => {
      if (xhr.readyState === 4) {
        if ((xhr.status === 200 || xhr.status === 0) && xhr.response !== null) {
          ...
          throw new Error(
            `Failed to load file! HTTP status: ${xhr.status}, file: ${url}`
          );
        }
      }
    };
  ...
  parse(node, buffer) {
    ...
    worker.onmessage = function (e) {
      ...
      for (let property in buffers) {
        ...
        if (
          parseInt(property) === Potree.PointAttributeNames.POSITION_CARTESIAN
        ) {
          ...
          parseInt(property) === Potree.PointAttributeNames.COLOR_PACKED
        ) {
          geometry.addAttribute(
            "color",
          ...
          parseInt(property) === Potree.PointAttributeNames.INTENSITY
        ) {
          geometry.addAttribute(
            "intensity",
          ...
          parseInt(property) === Potree.PointAttributeNames.CLASSIFICATION
        ) {
          geometry.addAttribute(
            "classification",
          ...
          parseInt(property) === Potree.PointAttributeNames.NORMAL_SPHEREMAPPED
        ) {
          geometry.addAttribute(
            "normal",
            new THREE.BufferAttribute(new Float32Array(buffer), 3)
          ...
          parseInt(property) === Potree.PointAttributeNames.NORMAL_OCT16
        ) {
          geometry.addAttribute(
            "normal",
            new THREE.BufferAttribute(new Float32Array(buffer), 3)
  ...
  parse(node, buffer) {
    ...
    lf.open()
      ...
      .then((v) => {
        ...
        let totalRead = 0;
        let totalToRead =
          skip <= 1 ? header.pointsCount : header.pointsCount / skip;
        let reader = function () {
...
Potree.PointCloudMaterial = class PointCloudMaterial extends THREE.RawShaderMaterial {
  constructor(parameters = {}) {
    ...
    this.visibleNodesTexture = Potree.utils.generateDataTexture(
      2048,
      1,
    ...
    this.gradientTexture = Potree.PointCloudMaterial.generateGradientTexture(
      this._gradient
    );
    this.lights = false;
    this.fog = false;
  ...
  recomputeClassification() {
    this.classificationTexture = Potree.PointCloudMaterial.generateClassificationTexture(
...
Potree.NormalizationEDLMaterial = class NormalizationEDLMaterial extends THREE.RawShaderMaterial {
  constructor(parameters = {}) {
    ...
    this.setValues({
      ...
      vertexShader: this.getDefines() + Potree.Shaders["normalize.vs"],
      fragmentShader:
        this.getDefines() + Potree.Shaders["normalize_and_edl.fs"],
    });
      ...
      if (e.button === THREE.MOUSE.LEFT) {
        if (noMovement) {
          let selectable = this.hoveredElements.find(
            (el) => el.object._listeners && el.object._listeners["select"]
          );
...
Potree.GeoControls = class GeoControls extends THREE.EventDispatcher {
  ...
  setTrackPos(trackPos, _preserveRelativeRotation) {
    ...
    if (newTangent.equals(oldTangent)) {
      ...
      let tangentDiffNormal = new THREE.Vector3()
        .crossVectors(oldTangent, newTangent)
        .normalize();
  ...
  pan(deltaX, deltaY) {
    ...
    if (this.object.fov !== undefined) {
      ...
      // orthographic
      this.panLeft(
        (deltaX * (this.object.right - this.object.left)) / element.clientWidth
      );
      this.panUp(
        (deltaY * (this.object.top - this.object.bottom)) / element.clientHeight
      );
    } else {
      // camera neither orthographic or perspective
      console.warn(
        "WARNING: GeoControls.js encountered an unknown camera type - pan disabled."
      );
    }
  }
  ...
  onMouseMove(event) {
    ...
    if (state === STATE.ROTATE) {
      ...
      this.rotateLeft(
        ((2 * Math.PI * rotateDelta.x) / element.clientWidth) * this.rotateSpeed
      );
  ...
  constructor(viewer) {
    ...
    let drag = (e) => {
      ...
      if (e.drag.mouse === Potree.MOUSE.LEFT) {
        ...
        let pivotToCam = new THREE.Vector3().subVectors(
          view.position,
          this.pivot
        );
...
Potree.Annotation = class extends THREE.EventDispatcher {
  constructor(args = {}) {
    ...
    this._display = true;
    this._expand = false;
    this.collapseThreshold = [args.collapseThreshold, 100].find(
      (e) => e !== undefined
    );
  ...
  hasView() {
    ...
    hasPosTargetView =
      hasPosTargetView && this.cameraPosition instanceof THREE.Vector3;
  ...
  moveHere(camera) {
    ...
    if (this.cameraPosition) {
      ...
      let endPosition = endTarget
        .clone()
        .add(direction.multiplyScalar(-this.radius));
  ...
  constructor(profile) {
    ...
    for (let i = 0; i < profile.points.length - 1; i++) {
      ...
      let center = new THREE.Vector3()
        .addVectors(endGround, startGround)
        .multiplyScalar(0.5);
      let length = startGround.distanceTo(endGround);
      let side = new THREE.Vector3()
        .subVectors(endGround, startGround)
        .normalize();
      let up = new THREE.Vector3(0, 0, 1);
      let forward = new THREE.Vector3().crossVectors(side, up).normalize();
      let N = forward;
      let cutPlane = new THREE.Plane().setFromNormalAndCoplanarPoint(
        N,
        startGround
      );
      let halfPlane = new THREE.Plane().setFromNormalAndCoplanarPoint(
        side,
        center
      );
...
Potree.ProfileRequest = class ProfileRequest {
    ...
    while (stack.length > 0) {
      ...
      if (node.level < this.maxDepth) {
        for (let i = 0; i < 8; i++) {
          ...
          if (
            child &&
            this.pointcloud.nodeIntersectsProfile(child, this.profile)
          ) {
            stack.push(child);
          }
        }
  ...
  *updateGenerator() {
    ...
    for (
      let i = 0;
      i < Math.min(maxNodesPerUpdate, this.priorityQueue.size());
      i++
    ) {
      ...
      if (node.loaded) {
        ...
        this.highestLevelServed = Math.max(
          node.getLevel(),
          this.highestLevelServed
        );

        let doTraverse =
          node.level % node.pcoGeometry.hierarchyStepSize === 0 &&
          node.hasChildren;
    ...
    if (intersectedNodes.length > 0) {
      for (let done of this.getPointsInsideProfile(
        intersectedNodes,
  ...
  *getAccepted(
    ...
    for (let i = 0; i < numPoints; i++) {
      pos.set(view[i * 3 + 0], view[i * 3 + 1], view[i * 3 + 2]);
      ...
      if (
        distance < this.profile.width / 2 &&
        centerDistance < segment.length / 2
      ) {
    ...
    for (let segment of target.segments) {
      for (let node of nodes) {
...

        {
          ...
          let bbWorld = node.boundingBox
            .clone()
            .applyMatrix4(this.pointcloud.matrixWorld);
          ...
          let closest = new THREE.Line3(start, end).closestPointToPoint(
            bsWorld.center,
            true
          );
        ...
        for (let result of this.getAccepted(
          numPoints,
          node,
          matrix,
          segment,
        ...
        let relevantAttributes = Object.keys(geometry.attributes).filter(
          (a) => !["position", "indices"].includes(a)
        );
...
Potree.PointCloudOctree = class extends Potree.PointCloudTree {
  ...
  computeVisibilityTextureData(nodes, camera) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      for (let j = 0; j < 8; j++) {
        ...
        if (
          child &&
          child.constructor === Potree.PointCloudOctreeNode &&
  ...
  getPointsInProfile(profile, maxDepth, callback) {
    if (callback) {
      let request = new Potree.ProfileRequest(
        this,
        profile,
        maxDepth,
        callback
    ...
    for (let i = 0; i < points.segments.length; i++) {
      ...
      let project = (function (_start, _end, _mileage, _boundingBox) {
        ...
        return function (position) {
          let toOrigin = new THREE.Matrix4().makeTranslation(
            -start.x,
            -boundingBox.min.y,
            -start.z
          );
          let alignWithX = new THREE.Matrix4().makeRotationY(-alpha);
          let applyMileage = new THREE.Matrix4().makeTranslation(
            mileage.x,
            0,
            0
          );
  ...
  *getFittedBoxGen(boxNode) {
    ...
    let fittedPosition = shrinkedLocalBounds
      .getCenter()
    ...
    let ds = new THREE.Vector3().subVectors(
  ...
  loadHierachyThenPoints() {
    ...
    let callback = function (node, hbuffer) {
      ...
      for (let i = 0; i < decoded.length; i++) {
        ...
        let currentNode = new Potree.PointCloudOctreeGeometryNode(
          name,
          pco,
          boundingBox
        );
...
Object.assign(
  Potree.PointCloudOctreeGeometryNode.prototype,
...
Potree.PointCloudGreyhoundGeometryNode = function (
  ...
  this.greyhoundOffset = this.pcoGeometry.offset
    .clone()
    .add(this.pcoGeometry.boundingBox.getSize().multiplyScalar(0.5));
};

Potree.PointCloudGreyhoundGeometryNode.IDCount = 0;

Potree.PointCloudGreyhoundGeometryNode.prototype = Object.create(
...
Potree.utils = class {
  static loadShapefileFeatures(file, callback) {
    ...
    shapefile.open(file).then((source) => {
...
Potree.Features = (function () {
  ...
  let gl =
    ftCanvas.getContext("webgl") || ftCanvas.getContext("experimental-webgl");
  if (gl === null) {
    return null;
  }
  ...
  update() {
    ...
    for (let i = 0; i <= lastIndex; i++) {
...

      {
        ...
        edgeLabel.setText(
          Potree.utils.addCommas(distance.toFixed(2)) +
            " " +
            this.lengthUnit.code
        );
        edgeLabel.visible =
          this.showDistances &&
...

      {
        ...
        let msg =
          Potree.utils.addCommas((angle * (180.0 / Math.PI)).toFixed(1)) +
          "\u00B0";
...

    {
      ...
      if (this.showHeight) {
        ...
        let msg =
          Potree.utils.addCommas(height.toFixed(2)) +
          " " +
          this.lengthUnit.code;
...

    {
      ...
      let msg =
        Potree.utils.addCommas(this.getArea().toFixed(1)) +
        " " +
        this.lengthUnit.code +
        "\u00B2";
...
Potree.TransformationTool = class TransformationTool {
    ...
    for (let handleName of Object.keys(this.focusHandles)) {
      ...
      pickSphere.addEventListener("click", (e) => {
        ...
        let handleLength = Math.abs(
          selected.scale.dot(new THREE.Vector3(...handle.alignment))
        );
        let alignment = new THREE.Vector3(...handle.alignment).multiplyScalar(
  ...
  initializeRotationHandles() {
    ...
    let outlineGeometry = new THREE.TorusGeometry(
      1,
      adjust * 0.04,
      8,
      64,
    ...
    let pickGeometry = new THREE.TorusGeometry(
      1,
      adjust * 0.1,
      6,
      4,
  ...
  dragRotationHandle(e) {
    ...
    n.copy(
      new THREE.Vector4(...localNormal.toArray(), 0).applyMatrix4(
  ...
  dragTranslationHandle(e) {
    ...
    if (!drag.intersectionStart && handle) {
      ...
      let dir = new THREE.Vector4(...handle.alignment, 0).applyMatrix4(
  ...
  dragScaleHandle(e) {
...

    {
      ...
      if (I) {
        ...
        let toObjectSpace = new THREE.Matrix4().getInverse(
          this.selection[0].matrixWorld
        );
        ...
        let dragDirection = dragDirectionOS.dot(
          new THREE.Vector3(...handle.alignment)
        );

        let diff = new THREE.Vector3().subVectors(iOnLine, drag.pivot);
        let diffScale = new THREE.Vector3(...handle.alignment).multiplyScalar(
  ...
  setActiveHandle(handle) {
    ...
    for (let handleName of Object.keys(this.scaleHandles)) {
      ...
      if (this.activeHandle === handle) {
        ...
        let relatedFocusHandle = this.focusHandles[
          handle.name.replace("scale", "focus")
        ];
        let relatedFocusNode = relatedFocusHandle.node;
        relatedFocusNode.setOpacity(0.4);

        for (let translationHandleName of Object.keys(
          this.translationHandles
        )) {
          let translationHandle = this.translationHandles[
            translationHandleName
          ];
  ...
  update() {
    if (this.selection.length === 1) {
      ...
      this.scene.scale.copy(
        selected.boundingBox.getSize().multiply(selected.scale)
      );
...

      {
        let axisScale = (alignment) => {
          let transformed = new THREE.Vector3(...alignment).applyMatrix4(
...
Potree.VolumeTool = class VolumeTool extends THREE.EventDispatcher {
  ...
  update() {
    ...
    for (let volume of volumes) {
      ...
      let text =
        Potree.utils.addCommas(volume.getVolume().toFixed(3)) + "\u00B3";
      label.setText(text);
    }
  }
...
Potree.ClippingTool = class ClippingTool extends THREE.EventDispatcher {
  constructor(viewer) {
    ...
    this.viewer.inputHandler.addEventListener("delete", (e) => {
      ...
      let polyVolumes = e.selection.filter(
        (e) => e instanceof Potree.PolygonClipVolume
      );
  ...
  startInsertion(args = {}) {
    ...
    let polyClipVol = new Potree.PolygonClipVolume(
      this.viewer.scene.getActiveCamera().clone()
    );
    ...
    let drop = (e) => {
      ...
      for (let pointcloud of this.viewer.scene.pointclouds) {
        ...
        let ray = new THREE.Ray(
          volCam.getWorldPosition(),
          volCam.getWorldDirection()
        );
        let rayInverse = new THREE.Ray(
          ray.origin
            .clone()
            .add(ray.direction.clone().multiplyScalar(volume.scale.z)),
      ...
      if (allPointsNear.length > 0 && allPointsFar.length > 0) {
        ...
        let closestOnLine = allPointsNear.map((p) =>
          viewLine.closestPointToPoint(p.position, false)
        );
        let closest = closestOnLine.sort(
          (a, b) => ray.origin.distanceTo(a) - ray.origin.distanceTo(b)
        )[0];
        ...
        let farthest = farthestOnLine.sort(
          (a, b) => ray.origin.distanceTo(b) - ray.origin.distanceTo(a)
        )[0];
...
Potree.ClipVolume = class extends THREE.Object3D {
  constructor(args) {
    ...
    this.add(this.frame);
    this.planeFrame = new THREE.LineSegments(
      planeFrameGeometry,
  ...
  offset(args) {
    ...
    if (axis === "x") {
      if (cs === "local") {
        this.position.add(
          this.localX.clone().multiplyScalar(dir * this.clipOffset)
        );
      } else if (cs === "global") {
      ...
      if (cs === "local") {
        this.position.add(
          this.localY.clone().multiplyScalar(dir * this.clipOffset)
        );
      } else if (cs === "global") {
      ...
      if (cs === "local") {
        this.position.add(
          this.localZ.clone().multiplyScalar(dir * this.clipOffset)
        );
      } else if (cs === "global") {
    ...
    if (cs === "local") {
      if (axis === "x") {
        this.rotateOnAxis(
          ...
          (dir * this.clipRotOffset * Math.PI) / 180
        );
      } else if (axis === "y") {
        this.rotateOnAxis(
  ...
  updateLocalSystem() {
    ...
    this.localX = new THREE.Vector3(1, 0, 0)
...
Potree.SpotLightHelper = class SpotLightHelper extends THREE.Object3D {
  ...
  update() {
    ...
    let target = new THREE.Vector3().addVectors(
      light.position,
      this.light.getWorldDirection().multiplyScalar(-1)
    );
...
`;
  ...
  static toString(measurements) {
    ...
    let points = measurements
      .filter((m) => m instanceof Potree.Measure)
      .map((m) => m.points)
...
Potree.PointCloudArena4DNode = class PointCloudArena4DNode extends Potree.PointCloudTreeNode {
  ...
  toTreeNode(child) {
    ...
    let sceneNode = THREE.PointCloud(
...
class EDLRenderer {
  ...
  render() {
    ...
    let querySkybox = Potree.startQuery(
      "EDL - Skybox",
    ...
    if (lights.length > 0 && !lights[0].disableShadowUpdates) {
      ...
      let queryShadows = Potree.startQuery(
        "EDL - shadows",
        viewer.renderer.getContext()
      );
    ...
    let queryColors = Potree.startQuery(
      "EDL - colorpass",
      viewer.renderer.getContext()
    );
    ...
    {
      // EDL OCCLUSION PASS
      let queryEDL = Potree.startQuery(
        "EDL - resolve",
        viewer.renderer.getContext()
      );
      ...
      if (this.screenshot) {
        Potree.utils.screenPass.render(
          viewer.renderer,
          this.edlMaterial,
    ...
    let queryRest = Potree.startQuery(
      "EDL - rest",
      viewer.renderer.getContext()
    );
...
class HQSplatRenderer {
  ...
  render() {
    ...
    let queryHQSplats = Potree.startQuery(
      "HQSplats",
      viewer.renderer.getContext()
    );
...
class RepRenderer {
  ...
  resize() {
    ...
    let needsResize =
      this.rtColor.width !== width || this.rtColor.height !== height;
  ...
  render() {
    ...
    if (!this.disableSnapshots) {
...

      {
        ...
        let needsResize =
          snap.target.width !== width || snap.target.height !== height;
      ...
      for (const octree of viewer.scene.pointclouds) {
        ...
        let from =
          ...
          (octree.visibleNodes.length / this.history.maxSnapshots);
        let to =
          (this.history.version + 1) *
          (octree.visibleNodes.length / this.history.maxSnapshots);
        ...
        viewer.pRenderer.renderOctree(octree, nodes, camera, snap.target, {
          vnTextureNodes: nodes,
        });
      }

      snap.camera = camera.clone();
      this.history.version =
        (this.history.version + 1) % this.history.maxSnapshots;
...

    {
      ...
      for (const octree of viewer.scene.pointclouds) {
        if (!this.disableSnapshots) {
          ...
          if (this.history.snapshots.length === this.history.maxSnapshots) {
            uniforms[`uSnapshot`].value = this.history.snapshots.map(
              (s) => s.target.texture
            );
            uniforms[`uSnapshotDepth`].value = this.history.snapshots.map(
              (s) => s.target.depthTexture
            );
            uniforms[`uSnapView`].value = this.history.snapshots.map(
              (s) => s.camera.matrixWorldInverse
            );
            uniforms[`uSnapProj`].value = this.history.snapshots.map(
              (s) => s.camera.projectionMatrix
            );
            uniforms[`uSnapProjInv`].value = this.history.snapshots.map((s) =>
              new THREE.Matrix4().getInverse(s.camera.projectionMatrix)
            );
            uniforms[`uSnapViewInv`].value = this.history.snapshots.map((s) =>
              new THREE.Matrix4().getInverse(s.camera.matrixWorld)
            );
          }
        } else {
          octree.material.snapEnabled = false;
...
Potree.View = class {
  ...
  lookAt(t) {
    ...
    if (arguments.length === 1) {
      ...
      V = new THREE.Vector3().subVectors(
        new THREE.Vector3(...arguments),
        this.position
      );
    }
  ...
  getPivot() {
    return new THREE.Vector3().addVectors(
      this.position,
      this.direction.multiplyScalar(this.radius)
    );
  }
...
Potree.Scene = class extends THREE.EventDispatcher {
  ...
  getBoundingBox(pointclouds = this.pointclouds) {
    ...
    for (let pointcloud of pointclouds) {
      ...
      let boxWorld = Potree.utils.computeTransformedBoundingBox(
        pointcloudBox,
        pointcloud.matrixWorld
      );
      box.union(boxWorld);
  ...
  getActiveCamera() {
    return this.cameraMode == Potree.CameraMode.PERSPECTIVE
      ? this.cameraP
      : this.cameraO;
  }
...
Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher {
  constructor(domElement, args = {}) {
    ...
    try {
      ...
      // start rendering!
      if (
        args.useDefaultRenderLoop === undefined ||
        args.useDefaultRenderLoop === true
      ) {
  ...
  setBackground(bg) {
    ...
    if (bg === "skybox") {
      this.skybox = Potree.utils.loadSkybox(
  ...
  setCameraMode(mode) {
    ...
    for (let pointcloud of this.scene.pointclouds) {
      pointcloud.material.useOrthographicCamera =
        mode == Potree.CameraMode.ORTHOGRAPHIC;
    }
  }
  ...
  loadSettingsFromURL() {
    if (Potree.utils.getParameterByName("pointSize")) {
      this.setPointSize(
        parseFloat(Potree.utils.getParameterByName("pointSize"))
      );
    }
    ...
    if (Potree.utils.getParameterByName("edlRadius")) {
      this.setEDLRadius(
        parseFloat(Potree.utils.getParameterByName("edlRadius"))
      );
    }

    if (Potree.utils.getParameterByName("edlStrength")) {
      this.setEDLStrength(
        parseFloat(Potree.utils.getParameterByName("edlStrength"))
      );
    }

    if (Potree.utils.getParameterByName("pointBudget")) {
      this.setPointBudget(
        parseFloat(Potree.utils.getParameterByName("pointBudget"))
      );
    }

    if (Potree.utils.getParameterByName("showBoundingBox")) {
      let enabled =
        Potree.utils.getParameterByName("showBoundingBox") === "true";
      if (enabled) {
  ...
  loadGUI(callback) {
    ...
    sidebarContainer.load(
      ...
      () => {
        ...
        imgMenuToggle.style = `margin-top: 50vh;margin-left: 3px;`;
        ...
        viewer.renderArea.insertBefore(
          imgMapToggle,
          viewer.renderArea.children[0]
        );
        viewer.renderArea.insertBefore(
          imgMenuToggle,
          viewer.renderArea.children[0]
        ...
        $(() => {
          ...
          let elProfile = $("<div>").load(
            ...
            () => {
              ...
              this.profileWindowController = new Potree.ProfileWindowController(
                this
              );
    ...
    this.scene.annotations.traverse((annotation) => {
...

      {
        ...
        if (viewer.scene.cameraMode == Potree.CameraMode.PERSPECTIVE) {
          ...
          screenSize = Potree.utils.projectedRadiusOrtho(
            radius,
  ...
  update(delta, timestamp) {
    ...
    this.scene.directionalLight.lookAt(
...

    {
      ...
      for (let pointcloud of this.scene.pointclouds.filter(
        (pc) => pc.visible
      )) {
          ...
          for (let attribute of Object.keys(point)) {
            ...
            if (attribute === "position") {
              let values = [...value].map((v) =>
                Potree.utils.addCommas(v.toFixed(3))
              );
              html += `
								<tr>
									<td>x</td>
...
initSidebar = (viewer) => {
  ...
  function initScene() {
    ...
    for (let volume of [
      ...
      ...viewer.scene.polygonClipVolumes,
    ]) {
  ...
  function initClippingTool() {
...

    {
      ...
      let currentClipTask = Object.keys(Potree.ClipTask).filter(
        (key) => Potree.ClipTask[key] === viewer.clipTask
      );
...

    {
      ...
      let currentClipMethod = Object.keys(Potree.ClipMethod).filter(
        (key) => Potree.ClipMethod[key] === viewer.clipMethod
      );
      ...
      clippingToolBar.append(
        createToolIcon(
          ...
          () => {
            if (
              !(
                viewer.scene.getActiveCamera() instanceof
  ...
  function initAppearance() {
    ...
    $("#sldEDLStrength").slider({
      ...
      step: 0.01,
      slide: function (event, ui) {
        viewer.setEDLStrength(ui.value);
      },
    });

    viewer.addEventListener("point_budget_changed", function (event) {
      $("#lblPointBudget")[0].innerHTML = Potree.utils.addCommas(
    ...
    viewer.addEventListener("background_changed", function (event) {
      $("input[name=background][value='" + viewer.getBackground() + "']").prop(
        "checked",
        true
      );
    });
  ...
  function initNavigation() {
    ...
    let elCameraProjection = $(`
			<selectgroup id="camera_projection_options">
				<option id="camera_projection_options_perspective" value="PERSPECTIVE">Perspectiva</option>
				<option id="camera_projection_options_orthigraphic" value="ORTHOGRAPHIC">Ortografica</option>
			</selectgroup>
		`);
    elNavigation.append(elCameraProjection);
    elCameraProjection.selectgroup({ title: "Tipo de Câmera" });
    elCameraProjection.find("input").click((e) => {
      viewer.setCameraMode(Potree.CameraMode[e.target.value]);
    });
    let cameraMode = Object.keys(Potree.CameraMode).filter(
      (key) => Potree.CameraMode[key] === viewer.scene.cameraMode
    );
    ...
    sldMoveSpeed.slider({
      ...
      step: 0.01,
      slide: function (event, ui) {
        viewer.setMoveSpeed(toLinearSpeed(ui.value));
      },
    });
  ...
  let initSettings = function () {
    {
      $("#sldMinNodeSize").slider({
        ...
        step: 0.01,
        slide: function (event, ui) {
          viewer.setMinNodeSize(ui.value);
        },
      });
  ...
  createCoordinatesTable(points) {
    ...
    for (let point of points) {
      ...
      this.elCopy.click(() => {
        ...
        this.viewer.postMessage(`Copied value to clipboard: <br>'${msg}'`, {
          duration: 3000,
        });
      });
...
class PointPanel extends MeasurePanel {
  ...
  update() {
    ...
    let elAttributesContainer = this.elContent.find(
      ".attributes_table_container"
    );
...
class VolumePanel extends MeasurePanel {
  constructor(viewer, measurement, propertiesPanel) {
    ...
    {
      // download
      this.elDownloadButton = this.elContent.find(
  ...
  download() {
    ...
    let pcs = [];
    for (let pointcloud of this.viewer.scene.pointclouds) {
      let urlIsAbsolute = new RegExp("^(?:[a-z]+:)?//", "i").test(
        pointcloud.pcoGeometry.url
      );
      let pc = "";
      if (urlIsAbsolute) {
    ...
    let pc = pcs.map((v) => `pointcloud[]=${v}`).join("&");
  ...
  update() {
    ...
    elCoordiantesContainer.empty();
    elCoordiantesContainer.append(
      this.createCoordinatesTable([this.measurement.position])
    );
...
class ProfilePanel extends MeasurePanel {
  constructor(viewer, measurement, propertiesPanel) {
    ...
    elShow2DProfile.click(() => {
      ...
      this.propertiesPanel.viewer.profileWindowController.setProfile(
        measurement
      );
    });
  ...
  update() {
    ...
    elCoordiantesContainer.empty();
    elCoordiantesContainer.append(
      this.createCoordinatesTable(this.measurement.points)
    );
  }
}
...
class CameraPanel {
  ...
  update() {
    ...
    let pos = camera.position
      .toArray()
      .map((c) => Potree.utils.addCommas(c.toFixed(3)));
...
Potree.PropertiesPanel = class PropertriesPanel {
  ...
  set(object) {
    ...
    if (object instanceof Potree.PointCloudTree) {
      ...
      object instanceof Potree.Profile ||
      object instanceof Potree.Volume
    ) {
  ...
  setPointCloud(pointcloud) {
    ...
    let panel = $(`
			<div class="scene_content selectable">
				...
				<li>
					<!--<label for="optMaterial" class="pv-select-label">Atributos:</label><br>-->
				...
				<div id="materials.composite_weight_container">
					...
					<li>Distancia: <span id="lblWeightIntensity"></span> <div id="sldWeightIntensity"></div>	</li>
					<li>Elevação: <span id="lblWeightElevation"></span> <div id="sldWeightElevation"></div>	</li>
					<li>Classificação: <span id="lblWeightClassification"></span> <div id="sldWeightClassification"></div>	</li>
				...
				<div id="materials.rgb_container">
					...
					<li>Gama: <span id="lblRGBGamma"></span> <div id="sldRGBGamma"></div>	</li>
					<li>Brilho: <span id="lblRGBBrightness"></span> <div id="sldRGBBrightness"></div>	</li>
				...
				<div id="materials.intensity_container">
					...
					<li>Distância ao Cabo: <span id="lblIntensityGamma"></span> <div id="sldIntensityGamma"></div>	</li>
    ...
    {
      // POINT SIZING
      let strSizeType = Object.keys(Potree.PointSizeType)[
...

    {
      ...
      let updateMaterialPanel = (event, ui) => {
        ...
        if (selectedValue === "Composição") {
          blockWeights.css("display", "block");
          blockElevation.css("display", "block");
          blockRGB.css("display", "block");
          blockIntensity.css("display", "block");
        } else if (selectedValue === "Elevação") {
          blockElevation.css("display", "block");
        } else if (selectedValue === "RGB e Elevação") {
          blockRGB.css("display", "block");
          blockElevation.css("display", "block");
        } else if (selectedValue === "RGB") {
          blockRGB.css("display", "block");
        } else if (selectedValue === "Cor") {
          blockColor.css("display", "block");
        } else if (selectedValue === "Distancia ao cabo") {
          blockIntensity.css("display", "block");
      ...
      let update = () => {
        attributeSelection
          .val(Potree.toMaterialName(material.pointColorType))
          .selectmenu("refresh");
      };
...

    {
      ...
      let updateHeightRange = function () {
        ...
        pointcloud.updateMatrixWorld(true);
        box = Potree.utils.computeTransformedBoundingBox(
          box,
          pointcloud.matrixWorld
        );
...
Potree.toInterleavedBufferAttribute = function toInterleavedBufferAttribute(
  pointAttribute
) {
  let att = null;
  ...
  if (pointAttribute.name === Potree.PointAttribute.POSITION_CARTESIAN.name) {
    ...
    pointAttribute.name === Potree.PointAttribute.CLASSIFICATION.name
  ) {
    att = new Potree.InterleavedBufferAttribute(
      "classification",
      4,
    ...
    pointAttribute.name === Potree.PointAttribute.NUMBER_OF_RETURNS.name
  ) {
    att = new Potree.InterleavedBufferAttribute(
      "numberOfReturns",
      4,
    ...
    pointAttribute.name === Potree.PointAttribute.NORMAL_SPHEREMAPPED.name
  ) {
    att = new Potree.InterleavedBufferAttribute(
      "normal",
      12,
